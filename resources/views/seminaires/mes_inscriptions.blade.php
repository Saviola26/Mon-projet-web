<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Mes Inscriptions aux Séminaires') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h3 class="text-lg font-semibold mb-4">Bonjour, {{ Auth::user()->name }} ! Voici vos inscriptions :</h3>
                    ...
                    @if ($seminairesInscrits->isEmpty())
                        <p class="text-gray-600 dark:text-gray-400">Vous n'êtes inscrit à aucun séminaire pour le moment.</p>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                    @foreach ($seminairesInscrits as $seminaire)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                {{-- Accède à la date d'inscription via la table pivot --}}
                                                <div class="text-sm text-gray-900 dark:text-gray-100">{{ \Carbon\Carbon::parse($seminaire->pivot->date_inscription)->format('d/m/Y H:i') }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $seminaire->resume }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="{{ route('seminaires.show', $seminaire) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-200">Voir les détails</a>
                                                ...
                                                @else
                                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800 ml-2">Terminé</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
